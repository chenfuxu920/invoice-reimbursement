...
///
/// The left side has 4 transport detail rows + 1 subtotal row (= 5 rows) plus an
/// overflow row for "市内交通费".  The right side has 4 hotel level rows
/// + 1 subtotal row (= 5 rows).  The section uses the larger count (6 rows total).
///
/// Row layout (0-indexed within this section, offset by 4 in the sheet):
///   idx 0 — 车船票 (left) / 战区级以上 (right)
///   idx 1 — 飞机票 (left) / 军级 (right)
///   idx 2 — 保险费 (left) / 师级 (right)
///   idx 3 — 订（退、改签）票 (left) / 其他人员 (right)
///   idx 4 — 小计 (left) / 小计 (right)
///   idx 5 — 市内交通费 (left) / 计发伙食补助费 (right)
fn write_transport_hotel_section(
    ...
    let transport_row_count: usize = 5; // 4 detail + 1 subtotal
...
mod tests {
    ...
    fn test_category_label() {
        ...
        assert_eq!(category_label(&InvoiceCategory::Insurance), "保险费");
        assert_eq!(
