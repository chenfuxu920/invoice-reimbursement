    ...
    for result in match_results {
        match &result.invoice.source {
            InvoiceSource::Manual => {
                ...
                blocks.push(OutputBlock::BlankInvoice {
                    payment: payment_text,
                });
            }
            InvoiceSource::Pdf(pdf_path) => {
                let img_path =
                    super::image_embedder::render_pdf_page_to_png(pdf_path, 0, output_dir, dpi)?;
                let rel = img_path
                    .file_name()
                    .unwrap_or_default()
                    .to_string_lossy()
                ...
                let payment_text = build_payment_text(result);
                blocks.push(OutputBlock::Invoice {
                    img: rel,
                    payment: payment_text,
                });
            }
        ...
        if matches!(result.invoice.category, InvoiceCategory::CityTransport) {
            ...
            if itin_idx < itinerary_pdfs.len() {
                ...
                let imgs =
                    super::image_embedder::render_pdf_all_pages_to_pngs(pdf, output_dir, dpi)?;
                for p in imgs {
                    itinerary_imgs.push(
                        p.file_name()
                            .unwrap_or_default()
            ...
            blocks.push(OutputBlock::Itinerary {
                imgs: itinerary_imgs,
                rows,
            });
        }
...
fn find_itinerary_pdfs(dir: &str, invoice_pdfs: &HashMap<String, &MatchResult>) -> Vec<String> {
