...
static PATTERNS: &[Pat] = &[
    Pat {
        re: r"(\d{4})[-/年](\d{1,2})[-/月](\d{1,2})[\s日]*(\d{1,2}):(\d{2}):(\d{2})",
        kind: Kind::FullSec,
    },
    Pat {
        re: r"(\d{4})[-/年](\d{1,2})[-/月](\d{1,2})[\s日]*(\d{1,2}):(\d{2})",
        kind: Kind::Full,
    },
    Pat {
        re: r"(\d{4})[-/年](\d{1,2})[-/月](\d{1,2})",
        kind: Kind::Date,
    },
    Pat {
        re: r"(\d{2})-(\d{2})[\s]*(\d{1,2})[:：][\s]*(\d{2})",
        kind: Kind::Short,
    },
    Pat {
        re: r"(\d{2})-(\d{2})(\d{1,2})[\s]+(\d{2})",
        kind: Kind::ShortMergedMin,
    },
    Pat {
        re: r"(\d{2})-(\d{2})(\d{1,2})",
        kind: Kind::ShortIncomplete,
    },
    Pat {
        re: r"(\d{2})-(\d{2})[\s]*(\d{1,2})[:：]",
        kind: Kind::ShortIncomplete,
    },
    Pat {
        re: r"(\d{2})-(\d{2})",
...
fn compiled() -> &'static Vec<(Regex, Kind)> {
    ...
    RE.get_or_init(|| {
        PATTERNS
            .iter()
            .map(|p| (Regex::new(p.re).unwrap(), p.kind))
            .collect()
    })
}
...
fn pad2(s: &str) -> String {
    s.parse::<u32>()
        .map_or_else(|_| s.to_string(), |n| format!("{n:02}"))
}
    ...
    for (re, kind) in compiled() {
        if let Some(c) = re.captures(text) {
            return Some(match kind {
                ...
                Kind::ShortMergedMin => {
                    format!("{}-{} {}:{}", &c[1], &c[2], pad2(&c[3]), pad2(&c[4]))
                }
...
mod tests {
    ...
    fn test_extract_full_datetime() {
        assert_eq!(
            extract_datetime("2026-04-24 17:58:59").as_deref(),
            Some("2026-04-24 17:58:59")
        );
    }
    ...
    fn test_extract_full_no_space() {
        assert_eq!(
            extract_datetime("2026-04-2408:48:00").as_deref(),
            Some("2026-04-24 08:48:00")
        );
    }
    ...
    fn test_extract_full_no_seconds() {
        assert_eq!(
            extract_datetime("2026-04-24 08:48").as_deref(),
            Some("2026-04-24 08:48")
        );
    }
    ...
    fn test_extract_date_only_chinese_and_slash() {
        assert_eq!(
            extract_datetime("2026/04/24").as_deref(),
            Some("2026-04-24")
        );
        assert_eq!(
            extract_datetime("2026年04月24日").as_deref(),
            Some("2026-04-24")
        );
    }
    ...
    fn test_extract_short_weekday_split_newline() {
        assert_eq!(
            extract_datetime("05-11 11:48 周\n一").as_deref(),
            Some("05-11 11:48")
        );
    }
    ...
    fn test_extract_short_colon_space() {
        assert_eq!(
            extract_datetime("04-22 21: 10").as_deref(),
            Some("04-22 21:10")
        );
    }
    ...
    fn test_extract_short_incomplete_minutes() {
        assert_eq!(
            extract_datetime("07-03 20:").as_deref(),
            Some("07-03 20:??")
        );
    }
    ...
    fn test_extract_short_merged_minutes() {
        assert_eq!(
            extract_datetime("07-0320 46").as_deref(),
            Some("07-03 20:46")
        );
    }
    ...
    fn test_extract_date_full() {
        assert_eq!(
            extract_date("2026-04-24 17:58:59").as_deref(),
            Some("2026-04-24")
        );
    }
