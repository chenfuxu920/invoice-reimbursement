...
/// 用于"先付费后开票"场景（保险费、退改签等无交易时间锚点的发票），
/// 避免匹配到开票日期之后的支付。
/// 无法解析时间的支付保留（不丢失候选，靠金额匹配兜底）。
pub fn filter_payments_by_date_direction(
    ...
    payments
        .iter()
        .filter(|p| {
            parse_payment_date(&p.transaction_time)
                .map(|pd| pd <= max_date)
                .unwrap_or(true)
        })
        .cloned()
...
mod tests {
    ...
    fn test_one_to_one_uses_travel_date_for_train() {
        ...
        let result = engine.match_one_to_one(&invoice, &payments).unwrap();
        // 用出行日期匹配，p_close 离 1/15 更近
        assert_eq!(result.payment_ids[0], "p_close");
    }
    ...
    #[test]
    fn test_one_to_one_train_travel_date_better_than_invoice_date() {
        let engine = MatchEngine::default();
        // 验证用出行日期比开票日期更准确：
        // 支付 1/14（出行前1天），开票 1/16（出行后1天）
        // 用出行日期 1/15 匹配：支付差1天
        // 若错误用开票日期 1/16：支付差2天
        // 两笔支付金额相同，应选出行日期附近的
        let mut invoice = make_invoice_at("inv1", 100.00, "2025-01-16");
        invoice.category = InvoiceCategory::Train;
        invoice.travel_date = Some(NaiveDate::from_ymd_opt(2025, 1, 15).unwrap());
        let payments = vec![
        ...
        let result = engine.match_one_to_one(&invoice, &payments).unwrap();
        // 用出行日期：p_before_travel 差1天，p_after_invoice 差2天 → 选 p_before_travel
        assert_eq!(result.payment_ids[0], "p_before_travel");
    }
    ...
    // === filter_payments_by_date_direction 测试 ===

    #[test]
    fn test_filter_keeps_payments_before_max_date() {
        let payments = vec![
            make_payment_at("p1", 100.0, "2025-01-10 12:00"),
            make_payment_at("p2", 50.0, "2025-01-14 08:30"),
        ];
        let max_date = NaiveDate::from_ymd_opt(2025, 1, 15).unwrap();
        let filtered = filter_payments_by_date_direction(&payments, max_date);
        assert_eq!(filtered.len(), 2);
    }

    #[test]
    fn test_filter_keeps_payment_on_same_day() {
        let payments = vec![
            make_payment_at("p1", 100.0, "2025-01-15 00:00"),
            make_payment_at("p2", 100.0, "2025-01-15 23:59"),
        ];
        let max_date = NaiveDate::from_ymd_opt(2025, 1, 15).unwrap();
        let filtered = filter_payments_by_date_direction(&payments, max_date);
        assert_eq!(filtered.len(), 2);
    ...
    #[test]
    fn test_filter_removes_payments_after_max_date() {
        let payments = vec![
            make_payment_at("p_before", 100.0, "2025-01-10 12:00"),
            make_payment_at("p_after", 100.0, "2025-01-16 12:00"),
            make_payment_at("p_way_after", 100.0, "2025-06-01 12:00"),
        ];
        let max_date = NaiveDate::from_ymd_opt(2025, 1, 15).unwrap();
        let filtered = filter_payments_by_date_direction(&payments, max_date);
        assert_eq!(filtered.len(), 1);
        assert_eq!(filtered[0].id, "p_before");
    }

    #[test]
    fn test_filter_keeps_unparseable_payments() {
        let payments = vec![
            make_payment_at("p_bad", 100.0, "无法解析的时间"),
            make_payment_at("p_good", 100.0, "2025-01-10 12:00"),
        ];
        let max_date = NaiveDate::from_ymd_opt(2025, 1, 15).unwrap();
        let filtered = filter_payments_by_date_direction(&payments, max_date);
        assert_eq!(filtered.len(), 2);
    ...
    #[test]
    fn test_filter_all_after_max_date() {
        let payments = vec![
            make_payment_at("p1", 100.0, "2025-01-16 12:00"),
            make_payment_at("p2", 100.0, "2025-02-01 12:00"),
        ];
        let max_date = NaiveDate::from_ymd_opt(2025, 1, 15).unwrap();
        let filtered = filter_payments_by_date_direction(&payments, max_date);
        assert!(filtered.is_empty());
