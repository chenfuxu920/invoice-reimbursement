...
#[allow(dead_code)]
fn generate_matching_data(invoice_count: usize, payment_count: usize) -> (Vec<Invoice>, Vec<PaymentRecord>) {
...
mod performance_tests {
    ...
    fn print_performance_metrics(result: &super::super::batch_optimizer::BatchMatchResult, duration: std::time::Duration) {
        ...
        println!("  High confidence matches: {}", result.high_confidence_matches().len());
    ...
    fn test_medium_scale_performance() {
        ...
        assert!(duration.as_millis() < 1000, "Expected < 1000ms, got {:?}", duration);
        assert!(result.matched.len() > 80, "Expected > 80 matches, got {}", result.matched.len());
    }
    ...
    fn test_one_to_many_matching_performance() {
        ...
        println!("  Total matched: {}", result.matched.len());
        println!("  One-to-One matches: {}", result.matched.iter().filter(|m| matches!(m.match_type, MatchType::OneToOne)).count());
        println!("  One-to-Many matches: {}", result.matched.iter().filter(|m| matches!(m.match_type, MatchType::OneToMany)).count());
        println!("========================================");
    ...
    fn test_performance_regression() {
        ...
        assert!(max_duration.as_millis() < 2000, "Expected < 2000ms, got {:?}", max_duration);
    }
    ...
    fn test_match_quality() {
        ...
        let medium_confidence: Vec<_> = result.matched.iter()
            .filter(|m| m.confidence >= 0.5 && m.confidence < 0.7)
            .collect();
        let low_confidence: Vec<_> = result.matched.iter()
    ...
    fn test_memory_efficiency() {
        ...
        println!("  Invoices: {}, Payments: {}", invoices.len(), payments.len());
...
mod data_generator_tests {
    ...
    fn test_invoice_category_distribution() {
        ...
        let categories: std::collections::HashSet<_> = invoices.iter().map(|i| i.category.clone()).collect();
    ...
    fn test_payment_merchant_distribution() {
        let payments = generate_test_payments(100);
        let merchants: std::collections::HashSet<_> = payments.iter().map(|p| p.merchant_name.clone()).collect();
