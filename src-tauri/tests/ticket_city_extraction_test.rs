...
use invoice_reimbursement_lib::pdf::text_extractor::{
    extract_pdf_column_aware, has_sufficient_text,
};
...
fn test_extract_cities_from_real_tickets() {
    let test_cases = vec![
        (
            "【飞猪】成都-长沙 机票",
            "../data/机票/【飞猪】成都-长沙  订单9586482810622-机票款凭证 报销凭证.pdf",
        ),
        (
            "【飞猪】长沙-成都 机票",
            "../data/机票/【飞猪】长沙-成都  订单9640359113622-机票款凭证 报销凭证.pdf",
        ),
    ...
    for (label, path) in &test_cases {
        ...
        let extraction = extract_pdf_column_aware(path)
            .unwrap_or_else(|e| panic!("  ✗ {} pdfplumber 文本提取失败: {}", label, e));
        {
            ...
            println!(
                "  pdfplumber 提取成功: {} 段文字, {} 字符",
                items.len(),
                char_count
            );

            assert!(
                has_sufficient_text(&items, 20),
                "  ✗ {} 文本不足 20 字符（提取质量回退？）",
                label
            );
            ...
            let inv = parse_invoice_text(&items, source)
                .unwrap_or_else(|e| panic!("  ✗ {} 解析失败: {}", label, e));
            {
