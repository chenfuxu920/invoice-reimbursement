...
> 本地优先的发票报销自动化桌面工具：发票 OCR 识别、支付账单智能匹配、按行程分趟归类、一键生成报销表单。
...
所有识别、解析与匹配均在本地完成，发票与账单数据不出本机。

---

## 功能特性

- **发票 OCR 识别**：基于本地 PaddleOCR v5，支持发票照片与 PDF 导入，自动提取金额、销售方、开票日期、发票号码等字段
- **行程单解析**：OCR 坐标表格解析与 PDF 纯文本提取双重交叉验证，兼容天府通、滴滴、高德、火车票等多样式行程单
...
- **离线优先**：OCR 模型随应用内置，数据全程不出本机；内置 GitHub Releases 自动更新

## 工作原理

```
发票（PDF/图片） ──▶ OCR + 文本解析 ──▶ 结构化发票
                                              │
...
行程单与发票的字段提取采用「OCR 坐标结果 + PDF 纯文本提取」双通道交叉验证，任一通道缺漏的字段由另一通道补全或修正，显著提升金额、时间、销售方等关键字段的准确率。
...
1. 导入发票 ──▶ 2. 导入账单 ──▶ 3. 智能匹配 ──▶ 4. 分趟归类 ──▶ 5. 生成报销
   │                │                │                │                │
   ▼                ▼                ▼                ▼                ▼
 拖拽照片/PDF    微信/支付宝      自动金额匹配     按城市/日期      PDF 表单
 OCR 自动识别    XLSX/CSV 导入    误差容忍 + 修正   人工归趟        + 对照清单
```
...
| OCR 引擎 | PaddleOCR v5（ONNX） | 本地推理，模型内置 |
